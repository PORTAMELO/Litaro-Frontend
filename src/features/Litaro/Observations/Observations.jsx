...
import { Button, MenuItem, Select, Typography, Chip, IconButton, Tooltip } from "@mui/material";
...
const Observations = () => {
  ...
  const selectedClassroom = mockFilters.classrooms.find((classroom) => classroom.id === filters.classroom);

  const filteredStudents = students.filter((student) => student.classroom === filters.classroom);

  const getStudent = (studentId) => students.find((student) => student.id === studentId);
  ...
  const handleGradeChange = (value) => {
    setFilters((prev) => ({
      ...prev,
      grade: value,
      classroom: mockFilters.classrooms.find((classroom) => classroom.grade === value)?.id ?? "",
    }));
  };

  const showNotification = ({ severity = "success", title = "", message = "" }) => {
    setNotification({
      open: true,
      severity,
      title,
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Registra observaciones y gestiona las citaciones de los estudiantes.</p>
        </div>
      </div>
        ...
        <div className={styles.tableHeader}>
          <div>
            ...
            <Typography className={styles.tableSubtitle}>
              Grupo {selectedClassroom?.name} · {filteredStudents.length} estudiantes
            </Typography>
          </div>
        </div>
      ...
      <section className={styles.recordsCard}>
        <div className={styles.tableHeader}>
          <div>
            <Typography className={styles.tableTitle}>Observaciones registradas</Typography>

            <Typography className={styles.tableSubtitle}>Historial de observaciones del grupo seleccionado.</Typography>
          </div>
        </div>
        ...
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            ...
            <tbody>
              {observations
                .filter((observation) => filteredStudents.some((student) => student.id === observation.studentId))
                .map((observation) => {
                  ...
                  return (
                    <tr key={observation.id}>
                      ...
                      <td className={styles.descriptionCell}>{observation.description}</td>

                      <td>
                        <Chip
                          label={observation.status}
                          size="small"
                          color={observation.status === "Con penalización" ? "warning" : "default"}
                        />
                      </td>
                      ...
                      <td>
                        <div className={styles.iconActions}>
                          <Tooltip title="Editar">
                            <IconButton size="small" color="primary" onClick={() => openObservation(student, observation)}>
                              <EditIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>

                          <Tooltip title="Eliminar">
                            <IconButton size="small" color="error" onClick={() => handleDeleteObservation(observation)}>
      ...
      <section className={styles.recordsCard}>
        <div className={styles.tableHeader}>
          <div>
            ...
            <Typography className={styles.tableSubtitle}>Citaciones programadas para los estudiantes.</Typography>
          </div>
        </div>
        ...
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            ...
            <tbody>
              {appointments
                .filter((appointment) => filteredStudents.some((student) => student.id === appointment.studentId))
                .map((appointment) => {
                    ...
                    <tr key={appointment.id}>
                      ...
                      <td>
                        <Chip
                          ...
                          size="small"
                          color={appointment.status === "Realizada" ? "success" : "warning"}
                        />
                      </td>
                      ...
                      <td>
                        <div className={styles.iconActions}>
                          <Tooltip title="Editar">
                            <IconButton size="small" color="primary" onClick={() => openAppointment(student, appointment)}>
                              <EditIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>

                          <Tooltip title="Eliminar">
                            <IconButton size="small" color="error" onClick={() => handleDeleteAppointment(appointment)}>
                              <DeleteIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>
                        </div>
